    ...
    public async Task<IActionResult> Create([Bind("BookingId,RoomId,CustomerName,CustomerEmail,CustomerPhone,CheckInDate,CheckOutDate,NumberOfGuests,SpecialRequests")] Booking booking)
    {
        if (ModelState.IsValid)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "La fecha de entrada debe ser anterior o igual a la fecha de salida.");
    ...
    {       
        ...
        // Validar que la fecha de entrada es anterior a la fecha de salida
        if (booking.CheckInDate >= booking.CheckOutDate)
        {
            ModelState.AddModelError(string.Empty, "La fecha de entrada debe ser anterior a la fecha de salida.");
            ViewBag.Rooms = new SelectList(_context.Rooms.Where(r => r.IsAvailable), "RoomId", "RoomType", booking.RoomId);
            return View(booking);
        }
